...
def getLLMResponse(query, age_option, tasktype_option, chat_history):
    llm = ChatGoogleGenerativeAI(temperature=.9, model="gemini-1.5-pro-latest")
    ...
    full_prompt = f"{chat_history}\n{new_prompt_template.format(template_userInput=query, template_ageoption=age_option, template_tasktype_option=tasktype_option)}"
    
    response = llm.invoke(full_prompt)
    return response.content

# UI Starts here
st.set_page_config(page_title="Koustav's GenX Engine - Marketing Wizard(Redstar)",
                   page_icon='✅',
                   layout='centered',
                   initial_sidebar_state='collapsed')

# Custom CSS for premium look and background
def add_custom_css():
    st.markdown("""
        <style>
        .stApp {
            background: url('https://wallpaperaccess.com/full/456291.jpg') ;
        ...
        .stMarkdown p, .stRadio>div>div>div>div {
            color: #ffffff;
        }
        
        stMarkdown h4, .stTextInput label, .stTextArea label, .stSelectbox label, .stSlider label {
...
st.header("Marketing Wizard: GenX Engine Pilot")

# Initialize chat history in session state
if "chat_history" not in st.session_state:
    st.session_state.chat_history = ""

form_input = st.text_area('Enter text', height=275, placeholder="Type your query here...")
...
if submit:
    user_input = form_input.strip()
    if user_input:
        # Get the response from the LLM
        response = getLLMResponse(user_input, age_option, tasktype_option, st.session_state.chat_history)
        # Update chat history
